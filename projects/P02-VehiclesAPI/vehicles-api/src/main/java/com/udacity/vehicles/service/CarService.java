...
public class CarService {
    ...
    public CarService(@Qualifier("maps") WebClient webClientMaps, @Qualifier("pricing") WebClient webClientPricing, CarRepository repository) {
    ...
    public Car findById(Long id) {
        ...
        Optional<Car> retrievedCar = this.repository.findById(id);
        if (retrievedCar.isPresent()) {
            Car car = retrievedCar.get();
            ...
            // Get address from webClientMaps and use it to update location object address
            Address address = webClientMaps.get().uri("/maps?lat={lat}&lon={lon}", latitude, longitude).retrieve().bodyToMono(Address.class).block();

            // Update the car location with the new address that was retrieved
